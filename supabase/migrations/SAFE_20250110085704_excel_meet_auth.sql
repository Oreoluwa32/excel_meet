-- Excel-meet Database Schema - Authentication and User Management (SAFE VERSION)
-- This version can be run multiple times without errors
-- Location: supabase/migrations/SAFE_20250110085704_excel_meet_auth.sql

-- 1. Create custom types (with IF NOT EXISTS check)
DO $$ BEGIN
...
-- 2. Create user_profiles table (with IF NOT EXISTS)
...
-- 3. Create essential indexes (with IF NOT EXISTS)
...
-- 6. Create RLS policies (drop and recreate to avoid conflicts)
...
-- 8. Create trigger for new user creation (drop and recreate)
...
-- 10. Create trigger for updated_at (drop and recreate)
DROP TRIGGER IF EXISTS update_user_profiles_updated_at ON public.user_profiles;
CREATE TRIGGER update_user_profiles_updated_at
    BEFORE UPDATE ON public.user_profiles
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- 11. Create mock data for testing (only if no data exists)
DO $$
DECLARE
